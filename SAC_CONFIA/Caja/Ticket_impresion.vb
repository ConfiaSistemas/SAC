...
Public Class Ticket_impresion
    ...
    Private Sub Ticket_Comision_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        AddHandler btnEfectivo.Click, AddressOf clickevennt
        AddHandler btnDeposito.Click, AddressOf clickevennt
        AddHandler btnTransferencia.Click, AddressOf clickevennt
    ...
    Private Async Sub ImprimeTicketPagoNormalAsync()
        Await Task.Factory.StartNew(Sub()
                                        If ObtenerTipoDoc("Liquidación Renovación") = tipoDoc Then
                                        Else

                                            Select Case btnMetodo.Text
                                                Case "Efectivo"
                                                    tipoDoc = ObtenerTipoDoc("Pago")
                                                Case "Depósito"
                                                    tipoDoc = ObtenerTipoDoc("Depósito")
                                                Case "Transferencia"
                                                    tipoDoc = ObtenerTipoDoc("Transferencia")
                                            End Select
                                        End If
    ...
    Private Sub btn_actualizar_Click(sender As Object, e As EventArgs) Handles btn_actualizar.Click
        ...
        ElseIf GetNombreDoc(tipoDoc) = "Terminal" Then
            ImprimeTicketPagoTerminalAsync()
        ...
        ElseIf GetNombreDoc(tipoDoc) = "Desempeño" Then
            ImprimeTicketEmpeño()
        ElseIf GetNombreDoc(tipoDoc) = "Promesa de pago" Or GetNombreDoc(tipoDoc) = "Promesa de pago legal" Or GetNombreDoc(tipoDoc) = "Promesa de pago convenio" Or GetNombreDoc(tipoDoc) = "Promesa de pago reestructura" Then
            ImprimeTicketPagoPromesaAsync()

        ElseIf GetNombreDoc(tipoDoc) = "Enganche Motocicleta" Then
            ImprimeTicketComisionYEngancheAsync()
        Else
    ...
    Private Sub txtRecibido_KeyDown(sender As Object, e As KeyEventArgs) Handles txtRecibido.KeyDown
        If e.KeyCode = Keys.F8 Then
            ...
            If autorizado Then
                AplicarDescuento.frmDescuento = Me
    ...
    Private Async Sub ImprimeTicketPagoConvenioAsync()
        Await Task.Factory.StartNew(Sub()
                                        Select Case btnMetodo.Text
                                            ...
                                            Case "Depósito"
                                                tipoDoc = ObtenerTipoDoc("Depósito Convenio")
                                            Case "Transferencia"
                                                tipoDoc = ObtenerTipoDoc("Transferencia Convenio")
                                        End Select
                                        Dim NumeroLetra As New NumLetra
                                        Dim tiempo As String = TimeOfDay.ToString("HH:mm:ss")
                                        If recibido < totalApagar Then
    ...
    Private Async Sub ImprimeTicketPagoReestructuraAsync()
        Await Task.Factory.StartNew(Sub()
                                        Select Case btnMetodo.Text
                                            ...
                                            Case "Depósito"
                                                tipoDoc = ObtenerTipoDoc("Depósito Reestructura")
                                            Case "Transferencia"
                                                tipoDoc = ObtenerTipoDoc("Transferencia Reestructura")
                                        End Select
                                        Dim NumeroLetra As New NumLetra
                                        Dim tiempo As String = TimeOfDay.ToString("HH:mm:ss")
                                        If recibido < totalApagar Then
    ...
    End Sub
    Private Async Sub ImprimeTicketPagoTerminalAsync()
        Await Task.Factory.StartNew(Sub()
                                        Select Case btnMetodo.Text
                                            Case "Efectivo"
                                                tipoDoc = ObtenerTipoDoc("Terminal")
                                            Case "Depósito"
                                                tipoDoc = ObtenerTipoDoc("Depósito Terminal")
                                            Case "Transferencia"
                                                tipoDoc = ObtenerTipoDoc("Transferencia Terminal")
                                        End Select
                                        Dim NumeroLetra As New NumLetra
                                        Dim tiempo As String = TimeOfDay.ToString("HH:mm:ss")
                                        If recibido < totalApagar Then
                                                              ...
                                                              For Each row As DataGridViewRow In inicio.dtimpuestos.Rows
                                                                  ...
                                                                  If c Then
                                                                      ...
                                                                      If saldo >= row.Cells(7).Value Then
                                                                          ...
                                                                          comandoAbonado = New SqlCommand
                                                                          Dim consultaAbonado As String
                                                                          consultaAbonado = "Update CalendarioConveniosTerminalesSac set abonado = '" & totalAbonado & "', pendiente = '0', estado = 'L',fecha = '" & fechainsercionhasta & "' where idpago = '" & row.Cells(1).Value & "'"
                                                                          comandoAbonado.Connection = conexionempresa
                                                                          comandoAbonado.CommandText = consultaAbonado
                                                                          comandoAbonado.ExecuteNonQuery()
                                                                      ...
                                                                      ElseIf saldo < row.Cells(7).Value And saldo > 0 Then
                                                                          ...
                                                                          Dim restante As Double
                                                                          restante = row.Cells(7).Value - saldo
                                                                          consultaAbonado = "Update CalendarioConveniosTerminalesSac set abonado = '" & totalAbonado & "', pendiente = '" & restante & "', fecha = '" & fechainsercionhasta & "' where idpago = '" & row.Cells(1).Value & "'"
                                                                          comandoAbonado.Connection = conexionempresa
                                                                          comandoAbonado.CommandText = consultaAbonado
                                                                          comandoAbonado.ExecuteNonQuery()
                                        ...
                                        Dim consultaAtraso As String
                                        Dim atraso As Double
                                        consultaAtraso = "select sum(pendiente) from CalendarioConveniosTerminalesSac where idConvenio = '" & convenio & "' and estado = 'V'"
                                        comandoAtraso = New SqlCommand
                                        comandoAtraso.Connection = conexionempresa
                                        comandoAtraso.CommandText = consultaAtraso
                                        Try
    ...
    End Sub
    Private Async Sub ImprimeTicketComisionYEngancheAsync()
        Await Task.Factory.StartNew(Sub()
                                        ...
                                        If recibido < totalApagar Then
                                            ...
                                            Cargando.SendToBack()
                                            Me.TopMost = False
                                            Dim result As DialogResult = MessageBox.Show("El pago recibido es menor al total a pagar, en un enganche no se puede recibir un monto menor al indicado",
                                                          "Title",
                                                          MessageBoxButtons.OK)
                                            btn_actualizar.Enabled = True
                                        Else
                                            ...
                                            fechasqlhasta = fechainsercionhasta
                                            fechainsercionhasta = Format(fechasqlhasta, "yyyy-MM-dd")
                                            consultarecibo = "Insert into Ticket values('" & total & "','" & recibido & "','" & txtCambio.Text & "','" & fechainsercionhasta & "','" & tiempo & "','" & idCreditoRecibo & "','','','" & tipoDoc & "','','" & nmusr & "','" & noCaja & "',0,'A','" & nm_completeusr & "','" & total & "') SELECT SCOPE_IDENTITY()"
                                            comandoRecibo = New SqlCommand
                                            comandoRecibo.Connection = conexionempresa
                                            comandoRecibo.CommandText = consultarecibo
                                            'comandoRecibo.ExecuteNonQuery()
                                            ...
                                            For copy As Integer = 1 To 2
                                                With P
                                                    ...
                                                    .GotoSixth(5)
                                                    .WriteChars((totalApagar / 1.16).ToString("$ ##,##00.00"))
                                                    .WriteLine("")
                                            ...
                                            Next
                                            Me.Invoke(Sub()
                                                          ComisionEnganche.BackgroundCobrados.RunWorkerAsync()
    ...
    Private Async Sub ImprimeTicketEmpeño()
        Await Task.Factory.StartNew(Sub()
                                        ...
                                        Else
                                            ...
                                            '    FileSystem.FileCopy("C:\ConfiaAdmin\SAC\COMPROBANTE DESEMPEÑO.docx", "C:\ConfiaAdmin\SAC\TEMPDOCS\TEMP_COMPROBANTE DESEMPEÑO.docx")
                                            '    Dim documento = DocX.Load("C:\ConfiaAdmin\SAC\TEMPDOCS\TEMP_COMPROBANTE DESEMPEÑO.docx")
                                            '    documento.ReplaceText("%%SUCURSAL%%", NombreEmpresa)
                                            '    documento.ReplaceText("%%FECHA%%", fechadePago)
                                            '    documento.ReplaceText("%%HORA%%", horadepago)
                                            '    documento.ReplaceText("%%FOLIO%%", idRecibo)
                                            '    documento.ReplaceText("%%CONTRATO%%", idCreditoRecibo)
                                            '    documento.ReplaceText("%%PRESTAMO%%", FormatCurrency(montocredito, 2))
                                            '    documento.ReplaceText("%%AVALUO%%", FormatCurrency((montocredito * 100) / 40, 2))
                                            '    documento.ReplaceText("%%INTERESES%%", FormatCurrency(interesEmpeño, 2))
                                            '    documento.ReplaceText("%%TOTAL%%", totalApagar)
                                            '    documento.ReplaceText("%%TOTAL%%", (NumeroLetra.Convertir(totalApagar.ToString, True)))


                                            '    documento.Save()
                                            '    documento.Dispose()
                                            '    Dim spDoc = New Spire.Doc.Document()
                                            '    spDoc.LoadFromFile("C:\ConfiaAdmin\SAC\TEMPDOCS\TEMP_COMPROBANTE DESEMPEÑO.docx")
                                            '    Dim dialog = New PrintPreviewDialog()
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        If abrir Then
            If heightFlow < 162 Then
                FlowLayoutPanel1.Height = heightFlow
                heightFlow += 10
            Else
                Timer1.Enabled = False
            End If
        Else
            If heightFlow >= 39 Then
                FlowLayoutPanel1.Height = heightFlow
                heightFlow -= 10
            Else
                Timer1.Enabled = False
